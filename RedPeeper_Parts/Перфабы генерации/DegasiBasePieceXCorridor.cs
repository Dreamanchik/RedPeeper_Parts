...
public class DegasiBasePieceXCorridor
{
    ...
    public static void Register()
    {
        ...
        _clone.ModifyPrefab += obj =>
        {
            ...
            // Убираю опоры
            _delete = _actualModel.transform.Find("BaseCorridorXShapeAdjustableSupport(Clone)").gameObject;
            GameObject.Destroy(_delete);

            // Я ненавижу эту дверь >:(
            _delete = _actualModel.transform.Find("BaseCorridorBulkhead(Clone)").gameObject;
            GameObject.Destroy(_delete);
